...
public class WorldGenNether extends WorldGenerator
	{
		...
		public WorldGenNether(int par1, int par2, int par3)
			{
				minableBlockId = par1;
				metadata = par2;
				numberOfBlocks = par3;
		...
		public boolean generate(World par1World, Random par2Random, int par3,
				int par4, int par5)
			{
				...
				double d1 = par3 + 8 - (MathHelper.sin(f) * numberOfBlocks)
						/ 8F;
				double d2 = par5 + 8 + (MathHelper.cos(f) * numberOfBlocks)
						/ 8F;
				double d3 = par5 + 8 - (MathHelper.cos(f) * numberOfBlocks)
						/ 8F;
										...
										for (int j2 = l; j2 <= k1; j2++)
											{
												double d14 = ((j2 + 0.5D) - d8)
														/ (d10 / 2D);
												if (d12 * d12 + d13 * d13 + d14
														* d14 < 1.0D
														&& par1World
																.getBlockId(l1,
																		i2, j2) == Block.netherrack.blockID)
													{
														par1World.setBlock(l1,
																i2, j2,
																minableBlockId,
																metadata, 2);
													}
